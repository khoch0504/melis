...
class ListController extends MelisAbstractActionController
{
    ...
    public function renderToolContentAction()
    {
        ...
        $melisTool->setMelisToolKey('testagainnn', 'testagainnn_tools');

        $columns = $melisTool->getColumns();
        $translator = $this->getServiceManager()->get('translator');
        $columns['actions'] = ['text' => $translator->translate('tr_testagainnn_common_table_column_action')];

        $view->tableColumns = $columns;
        $view->toolTable = $melisTool->getDataTableConfiguration('#testAgainnnTableContent', true, false, ['order' => [[ 0, 'desc' ]]]);
    ...
    public function getListAction()
    {
        ...
        if($this->getRequest()->isPost()){
            ...
            $tableData = $testAgainnnService->getList($start, $length, $melisTool->getSearchableColumns(), $search, $selCol, $sortOrder, $langId)->toArray();
            $dataCount = $testAgainnnService->getList(null, null, $melisTool->getSearchableColumns(), $search, null, 'ASC', $langId, true)->current();
    ...
    {
        $this->getEventManager()->trigger('testagainnn_delete_end', $this, $this->getRequest());

        return new JsonModel([
            'success' => true,
            'textTitle' => 'tr_testagainnn_delete_item',
            'textMessage' => 'tr_testagainnn_delete_success',
        ]);
    }
}